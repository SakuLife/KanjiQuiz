...
echo [1/3] Activating virtual environment... >> "%LOG_FILE%"
...
REM Step 2: Run video creation
echo [2/3] Running video creation... >> "%LOG_FILE%"
echo [2/3] Running video creation...
...
if %REPORT_EXIT% NEQ 0 (
    ...
    echo WARNING: Analysis failed with exit code %REPORT_EXIT%
) else (
...
if %APP_EXIT% EQU 0 if %REPORT_EXIT% EQU 0 (
    ...
    echo STATUS: Some tasks completed with warnings - check log for details
    set FINAL_EXIT=1
)
